...
case "$BOOKDROP_FOLDER" in
    /*|*..*)
        echo "Invalid bookdrop_folder: must be relative to /media" >&2
        exit 1
        ;;
esac
...
bashio::log.info "Bookdrop folder: /media/$BOOKDROP_FOLDER"
bashio::log.info "Books folder: /share/$BOOKS_FOLDER"
...
chown -R "$USER_ID:$GROUP_ID" /data /bookdrop /books 2>/dev/null || true
